...
def check():
	if cmds.optionVar(ex="QU_SafetyProjectionAxis"):
		...
		cmds.optionVar(iv=("QU_SafetyProjectionAxis",3))
	...
	if cmds.optionVar(ex="QU_TexelDensity"):
		pass
	else:
		cmds.optionVar(fv=("QU_TexelDensity",20.48))
	...
	if cmds.optionVar(ex="QU_StackSimilarShells"):
		pass
	else:
		cmds.optionVar(sv=("QU_StackSimilarShells","False"))